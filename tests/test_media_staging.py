...
async def test_approval_cache_coalesces_repeat_ask(test_user: User) -> None:
    ...
    await store.reset_permissions_async(test_user.id)
...
async def test_always_allow_for_upload_to_storage_persists_globally(
    ...
    level_global = await store.check_permission_async(
        test_user.id, "upload_to_storage", default=PermissionLevel.ASK
    )
    level_different_client = await store.check_permission_async(
...
async def test_permissions_path_match_is_case_insensitive(test_user: User) -> None:
    ...
    await store.set_permission_async(
        test_user.id, "qb_query", PermissionLevel.ALWAYS, resource="Invoice"
    )
