    ...
    return [
        Tool(
            ...
            usage_hint=(
                ...
                f"Connection-flow protocol -- follow these steps in order, do not "
                f"skip step 1: "
                f"(1) Before declaring whether an integration is connected, and "
                f"before offering ANY connect link, call action='status' to read "
                f"the live state. Never assert connection state from USER.md, "
                f"MEMORY.md, or earlier tool results in this conversation; the "
                f"user may have OAuthed or revoked since those were written. "
                f"(2) If status shows the target already connected, skip the "
                f"connect step entirely and do not re-prompt for something they "
                f"already set up. "
                f"(3) Only when status confirms not connected, call "
                f"action='connect' with a target from the list above to mint a "
                f"fresh OAuth link the user can tap. For 'appfolio_vendor' you "
                f"will get magic-link instructions instead of a URL; follow them "
                f"and then call appfolio_connect directly. "
                f"If the user reports that an OAuth link 'never arrived' or "
                f"'didn't send', do NOT re-paste the URL from earlier in this "
                f"conversation -- the link's one-time state token may already be "
                f"consumed, or the user may have completed the flow silently. "
                f"Restart at step 1: call action='status' first; if still not "
                f"connected, call action='connect' again to mint a fresh link. "
