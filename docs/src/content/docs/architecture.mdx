...
- **File-based storage**: all data in JSON, JSONL, and Markdown files under `data/users/`. No database required
...
All data is stored as files under `data/users/`. Each user gets an isolated directory:
...
data/
  ...
  users/
    {id}/
      ...
      BOOTSTRAP.md                 # First-run onboarding prompt (deleted after onboarding)
      memory/
