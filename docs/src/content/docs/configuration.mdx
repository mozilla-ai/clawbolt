...
When `QUICKBOOKS_CLIENT_ID` and `QUICKBOOKS_CLIENT_SECRET` are set, users can connect their QuickBooks account via the web dashboard. Once connected, the agent gains a specialist `qb_query` tool for running read-only queries against QuickBooks Online.
