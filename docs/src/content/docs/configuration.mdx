...
| `QUICKBOOKS_REALM_ID` | | Company realm (company) ID from QuickBooks |
| `QUICKBOOKS_ACCESS_TOKEN` | | Current OAuth access token (auto-refreshed at runtime) |
| `QUICKBOOKS_REFRESH_TOKEN` | | OAuth refresh token for access token renewal |
| `QUICKBOOKS_ENVIRONMENT` | `sandbox` | `sandbox` for testing or `production` for live data |

When configured, the agent gains specialist tools to search items/customers, create invoices, and send them via email through QuickBooks Online.
