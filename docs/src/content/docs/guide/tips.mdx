...
The web dashboard shows this information in a separate tool-call panel, so the confirmation block only appears on text-based channels (iMessage, SMS, Telegram).
