...
| `LLM_PROMPT_CACHE` | `auto` | Whether to stamp Anthropic `cache_control` breakpoints: `auto`, `always`, or `never`. `auto` marks only when the provider serves the Messages API natively and the request reaches that provider, so setting `LLM_API_BASE` on the `anthropic` provider turns marking off: a gateway picks its downstream provider from the model string, and one fronting a non-Anthropic model rejects a marked request with a 400 rather than ignoring the marker. Set `always` when the endpoint is Anthropic-backed for every model it serves, to keep caching through it. `never` forgoes prompt caching entirely. The `azureanthropic` and `vertexaianthropic` providers are unaffected by the endpoint check, since their base names your own resource |
